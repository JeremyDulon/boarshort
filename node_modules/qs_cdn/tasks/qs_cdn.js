...
module.exports = function(grunt) {
	...
	var reghtmls = [
		new RegExp(/url\(['"]?(?!.*\/\/)([^'"]+)['"]?\)/ig),
		new RegExp(/<(?:img|link|source|script).*\b(?:href|src)=['"](?!.*\/\/)(?!.*staticlink)([^'"\{]+)['"].*\/?>/ig),
		new RegExp(/<(?:a).*\b(?:href)=['"](?!.*\/\/)(?!.*staticlink)([^'"\{]+)['"].*data-cdn.*\/?>/ig),
		new RegExp(/<script.*\bdata-main=['"](?!.*\/\/)(?!.*staticlink)([^'"\{]+)['"].*\/?>/ig)
	];
  ...
  var regcssfilter = new RegExp(/filter[\w\.\:]+\(src=['"]([^'"]+)['"]/ig);
	...
	grunt.registerMultiTask('cdn', "Properly prepends a CDN url to those assets referenced with absolute paths (but not URLs)", function() {
		...
		var suffix = this.options().link_suffix || '';
		...
		files.forEach(function(file) {
			file.src.forEach(function (filepath) {
				...
				if (supportedTypes[type] == "html") {
					content = html.call(self, content, filepath, relativeTo, suffix);
				} else if (supportedTypes[type] === "css") {
					content = css.call(self, content, filepath, relativeTo, suffix);
				}
	...
	function html(content, filename, relativeTo, suffix) {
        var self = this;
		return reghtmls.reduce(function (value, reghtml) {
			...
			value = value.replace(reghtml, function(match, resource) {
				return match.replace(resource, cdnUrl.call(self, resource, filename, relativeTo, suffix));
			});
	...
	function css(content, filename, relativeTo, suffix) {
        var self = this,
            getUrl = function (resource) {
        			resource = resource.replace(/^['"]/, '').replace(/['"]$/, '');
        			var url = cdnUrl.call(self, resource, filename, relativeTo, suffix);
              return url;
            };
	...
	function cdnUrl(resource, filename, relativeTo, suffix) {
        var options = this.options();
		// skip those absolute urls
		if (resource.match(/^https?:\/\//i) || resource.match(/^\/\//) || resource.match(/^data:/i) || resource.match(/staticlink/)) {
